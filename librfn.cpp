...
template<class OP>
int calculate_W_impl_invertMxM(OP& op, const float* W, const float* P, float* Wout, const int k, const int m,
...
template<class OP>
int calculate_W_impl_invertKxK(OP& op, const float* W, const float* Pinv, float* Wout, const int k, const int m,
...
int train(const float* X_host, float* W_host, float* P_host, const int n, const int m, const int k, const int n_iter,
		...
		const float momentum, const int input_noise_type, const int activation_type, const int apply_scaling,
	...
	for (int cur_iter = 0; cur_iter < n_iter; ++cur_iter) {
		...
		for (int cur_batch = 0; cur_batch < n_batches; ++cur_batch) {
			...
			case 1:
				op.maximum(H, h_threshold, batch_size * k);
				break;
			case 2:
				op.leaky_relu(H, h_threshold, batch_size * k);
				break;
			case 3:
				op.sigmoid(H, batch_size * k);
				break;
			case 4:
				op.tanh(H, batch_size * k);
				break;
			default:
				printf("invalid noise type");
				assert(false);
...
template<class OP>
void calculate_W(const float* X_host, const float* W_host, const float* P_host, float* Wout_host, const int n,
		const int m, const int k, const int activation_type, const int apply_scaling, const float h_threshold,
		int gpu_id) {
	...
	switch (activation_type) {
	case 1:
		op.maximum(H, h_threshold, n * k);
		break;
	case 2:
		op.leaky_relu(H, h_threshold, n * k);
		break;
	case 3:
		op.sigmoid(H, n * k);
		break;
...
int train_cpu(const float* X, float* W, float* P, const int n, const int m, const int k, const int n_iter,
		int batch_size, const float etaW, const float etaP, const float minP, const float h_threshold,
		const float dropout_rate, const float input_noise_rate, const float l2_weightdecay, const float l1_weightdecay,
		const float momentum, const int input_noise_type, const int activation_type, const int apply_scaling,
		const int applyNewtonUpdate, unsigned long seed) {
	if (k > m) {
		return train<CPU_Operations, true>(X, W, P, n, m, k, n_iter, batch_size, etaW, etaP, minP, h_threshold,
				dropout_rate, input_noise_rate, l2_weightdecay, l1_weightdecay, momentum, input_noise_type,
				activation_type, apply_scaling, applyNewtonUpdate, seed, -1);
	} else {
		return train<CPU_Operations, false>(X, W, P, n, m, k, n_iter, batch_size, etaW, etaP, minP, h_threshold,
				dropout_rate, input_noise_rate, l2_weightdecay, l1_weightdecay, momentum, input_noise_type,
				activation_type, apply_scaling, applyNewtonUpdate, seed, -1);
	}
}
...
#ifndef NOGPU
int train_gpu(const float* X_host, float* W_host, float* P_host, const int n, const int m, const int k,
		const int n_iter, int batch_size, const float etaW, const float etaP, const float minP, const float h_threshold,
		const float dropout_rate, const float input_noise_rate, const float l2_weightdecay, const float l1_weightdecay,
		const float momentum, const int input_noise_type, const int activation_type, const int apply_scaling,
		const int applyNewtonUpdate, unsigned long seed, const int gpu_id) {
	if (k > m) {
		return train<GPUDenseOperations, true>(X_host, W_host, P_host, n, m, k, n_iter, batch_size, etaW, etaP, minP,
				h_threshold, dropout_rate, input_noise_rate, l2_weightdecay, l1_weightdecay, momentum, input_noise_type,
				activation_type, apply_scaling, applyNewtonUpdate, seed, gpu_id);
	} else {
		return train<GPUDenseOperations, false>(X_host, W_host, P_host, n, m, k, n_iter, batch_size, etaW, etaP, minP,
				h_threshold, dropout_rate, input_noise_rate, l2_weightdecay, l1_weightdecay, momentum, input_noise_type,
				activation_type, apply_scaling, applyNewtonUpdate, seed, gpu_id);
	}
}
...
void calculate_W_cpu(const float* X, const float* W, const float* P, float* Wout, const int n, const int m, const int k,
...
#ifndef NOGPU
void calculate_W_gpu(const float* X, const float* W, const float* P, float* Wout, const int n, const int m, const int k,
		const int activation_type, const int apply_scaling, const float h_threshold, int gpu_id) {
