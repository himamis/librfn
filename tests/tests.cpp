...
TEST_CASE( "Variance of CPU/GPU on large matrices", "[cpu_vs_gpu]" ) {
	...
	GPUSparseOperations gpu_sparse(m, n, m, 0, -1);
	...
	sparse_matrix_csr sparse_mat = gpu_sparse.dense_to_sparse_csr(X_d, m, n);

	float* var_h = cpu_op.malloc(m * sizeof(float));
	float* var_d = gpu_op.malloc(m * sizeof(float));
	float* var_sp = gpu_op.malloc(m * sizeof(float));
	cpu_op.calculate_column_variance(X_h, n, m, var_h);
	gpu_op.calculate_column_variance(X_d, n, m, var_d);
	gpu_sparse.calculate_column_variance(sparse_mat, n, m, var_sp);
	float* var_gpu_h = cpu_op.malloc(m * sizeof(float));
	float* var_sparse_h = cpu_op.malloc(m * sizeof(float));
	gpu_op.to_host(var_d, var_gpu_h, m * sizeof(float));
	gpu_op.to_host(var_sp, var_sparse_h, m * sizeof(float));

	for (unsigned i = 0; i < m; ++i) {
		CHECK(abs(var_h[i] - var_gpu_h[i]) < 1e-3);
		CHECK(abs(var_h[i] - var_sparse_h[i]) < 1e-3);
	}
	cpu_op.free(var_h);
	cpu_op.free(var_sp);
